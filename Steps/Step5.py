...
def process_svg_file(input_file):
    try:
        ...
        orange_pattern = r'<path[^>]*?id="(path\d+)"[^>]*?style="[^"]*?stroke:#fb7905[^"]*"[^>]*?d="([^"]*)"'
        matches = re.finditer(orange_pattern, svg_text)
        modified_svg = svg_text
        ...
        if all_matched_pairs:
            ...
            print("\nUpdating colors...")
            # Now update the colors for all matched paths
            for pair in all_matched_pairs:
                modified_svg = update_path_color(modified_svg, pair[0])  # Update first path
                modified_svg = update_path_color(modified_svg, pair[1])  # Update second path
